...
# Check
paste0("f.type-",c("Civil","Private","SelfEm","Other"))

# Define f.type as a factor and use 'nice' level names
df$f.type<-factor(df$f.type,levels = 1:4, labels = paste0("f.type-",c("Civil","Private","SelfEm","Other")))
...
df$f.education<-factor(df$f.education,levels = 1:6, labels = paste0("education-",c("No-HS-grad","HS-grad","Some-college","Assoc-Degree_and_Prof","Bachelors-Degree","Professional_Degree")))
...
paste0("occupation-",c("Sales","Service","Specialists", "Office_Workers", "Logistics&Maintenance"))

df$f.occupation<-factor(df$f.occupation,levels = 1:5, labels = paste0("occupation-",c("Sales","Service","Specialists", "Office_Workers", "Logistics&Maintenance")))

# Comprobar nueva variable
summary(df$f.occupation)
barplot((table(df$f.occupation)))
...
paste0("relationship-",c("Not_Married","Married", "Own-child"))

df$f.relationship<-factor(df$f.relationship,levels = 1:3, labels = paste0("relationship-",c("Not_Married","Married", "Own-child")))
...
#f.edu-num -> factor age
cut(df$`capital-gain`,quantile(df$`capital-gain`))
df$f.capGain<-factor(cut(df$`capital-gain`,quantile(df$`capital-gain`),include.lowest = T))
summary(df$f.capGain)

# Reasonable according to target?
tapply(df$`capital-gain`,df$f.capGain,median) # not OK

# Alternativa para otra situación
df$f.capGain<-factor(cut(df$`capital-gain`,c(0,1,100000),include.lowest = T))
summary(df$f.capGain)
barplot(table(df$f.capGain))
#tapply(df$`capital-gain`,df$f.capGain,mean)
...
#f.edu-num -> factor age
cut(df$`capital-loss`,quantile(df$`capital-loss`))
df$f.capGain<-factor(cut(df$`capital-loss`,quantile(df$`capital-loss`),include.lowest = T))
summary(df$f.capLoss)

# Reasonable according to target?
tapply(df$`capital-loss`,df$f.capLoss,median) # not OK

# Alternativa para otra situación
df$f.capLoss<-factor(cut(df$`capital-loss`,c(0,1,100000),include.lowest = T))
summary(df$f.capLoss)
barplot(table(df$f.capLoss))
#tapply(df$`capital-loss`,df$f.capLoss,mean)
...
#f.edu-num -> factor age
cut(df$`hours-per-week`,quantile(df$`hours-per-week`))
df$f.hoursPW<-factor(cut(df$`hours-per-week`,quantile(df$`hours-per-week`),include.lowest = T))
summary(df$f.hoursPW)

# Reasonable according to target?
tapply(df$`hours-per-week`,df$f.hoursPW,median) # not OK

# Alternativa para otra situación
df$f.hoursPW<-factor(cut(df$`hours-per-week`,c(0,30,39,40,50,100),include.lowest = T))
summary(df$f.hoursPW)
barplot(table(df$f.hoursPW))
...
abline(h=outsua, col= "red",lwd=1, lty= 2)
